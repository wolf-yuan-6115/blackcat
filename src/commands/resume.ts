      ...
      if (currentPlayer) {
        if (!currentPlayer.status.paused) {
          const alreadyResumeEmbed = new EmbedBuilder()
            .setTitle("❌ Current song is already playing")
            .setDescription("Use `/pause` to pause current song")
            .setColor(colors.danger);
          interaction
            .reply({ embeds: [alreadyResumeEmbed] })
            // eslint-disable-next-line @typescript-eslint/no-empty-function
            .catch(() => {});
        } else {
          return currentPlayer.resume(interaction);
        }
      }
    }
  },
